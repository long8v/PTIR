...
- **data :** Vero-600K = 59 dataset, 6 카테고리 × 100K
...
- SFT vs RL ablation: 같은 Vero-600K 에 대해 RL 이 SFT 보다 +4.4 점 
...
- ablation: 
  ...
  - math-verify 단일 reward 51.8 → multi-route 57.2 (Table 4b). task-routed 가 +5.4 절대점수 차이로 이김
...
- Vero-MiMo-7B vs MiMo-VL-7B-RL (closed RL recipe): 6 카테고리 중 STEM +0.5, Knowledge +5.1, Captioning +4.0 으로 3개 이김 
...
- 카테고리별 reasoning length 차이가 큼: Spatial & Action 평균 1983 단어 vs Grounding/Search 평균 125 단어
  ...
  - 재밌게도 STEM 보다 Spatial & Action이 더 문장이 많이 필요하네
