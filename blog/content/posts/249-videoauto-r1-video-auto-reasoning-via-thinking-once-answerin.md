...
summary: "video + think — Ablation studies show that \"always-think\" isn't the answer. However, it's more accurate to say that auto-mode is efficient rather than that it improves absolute performance. The framing of \"confidence-based early-exit gating\" is elegant."
---
...
- **Data:** RL 83K (8 rollouts removed from 137K—all-correct and all-wrong cases). text 6.4K (DAPO-Math) / image 27.5K (ViRL, ThinkLite-Hard) / video 49.4K (Video-R1, TVBench, STI-Bench, MMR-VBench, Charades-STA, ActivityNet, Time-R1, NExT-GQA)
...
- **Contribution:** The ablation results demonstrate that "always-think" is not the answer. However, it is more accurate to view the auto-mode as efficient rather than one that improves absolute performance. The framing of "confidence-based early-exit gating" is elegant.
...
- No "think" or "no-think" labeling during training — gating is determined only at inference time. Existing approaches like AdaptThink explicitly mix "think" and "no-think" samples during on-policy training, but this is said to have issues with data balancing and hyperparameter sensitivity.
- reward
...
- MVBench (perception-focused) — mean probability 94.8, think 25%, gain +0.1
